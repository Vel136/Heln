...
type function FireSignature(signal: type, signature: type, fallback: type): type
	...
	local returns = signature:returns()
	return types.newfunction(params, {head = returns.head, tail = returns.tail})
end
...
type function WaitSignature(signal: type, signature: type, fallback: type): type
	...
	local waitParams = {head = selfParam, tail = nil :: type?}
	local returns = signature:returns()
	return types.newfunction(waitParams, {head = returns.head, tail = returns.tail})
end
